...
class Client extends Base {
  ...
  _invoke(operation, args, location, callback, options, extraHeaders) {
    ...
    options = _.omit(options, dynamicSoapHeaderProperty); //Add extra headers to http request
    ...
    debug('client request. headers: %j', headers); //Unlike other security objects, NTLMSecurity is passed in through client options rather than client.setSecurity(ntlmSecurity) as some
    ...
    var soapBodyElement = envelope.body; //add soapHeaders to envelope. Header can be xml, or JSON object which may or may not be described in WSDL/XSD.
    ...
    var prettyPrint = true; //some web services don't accept request envelope with newlines and indentations in which case user has to set {prettyPrint: false} as client option

    if (self.httpClient.options && self.httpClient.options.prettyPrint !== undefined) {
      prettyPrint = self.httpClient.options.prettyPrint;
    }

    message = envelope.body.toString({
      pretty: prettyPrint
    });
    xml = envelope.doc.end({
      pretty: prettyPrint
    });
    ...
    req = self.httpClient.request(location, xml, function (err, response, body) {
      ...
      if (err) {
        ...
        try {
          ...
          if (!output) {
            debug('client response. output not present'); //  If the response is JSON then return it as-is.
    ...
    }, headers, options, self); // Added mostly for testability, but possibly useful for debugging
