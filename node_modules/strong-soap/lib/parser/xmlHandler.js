...
class XMLHandler {
  ...
  jsonToXml(node, nsContext, descriptor, val) {
    ...
    if (descriptor instanceof ElementDescriptor) {
      ...
      if (descriptor.form === 'unqualified') {
        ...
        nsContext.pushContext();
        nameSpaceContextCreated = true; // get the mapping for the namespace uri

        let mapping = nsContext.getPrefixMapping(descriptor.qname.nsURI);
        let newlyDeclared = false; // if namespace not declared, declare it
        ...
        let prefix = mapping ? mapping.prefix : descriptor.qname.prefix;
        elementName = prefix ? prefix + ':' + name : name; // if namespace is newly declared add the xmlns attribute
      ...
      if (isSimple && /<!\[CDATA/.test(val)) {
        ...
        if (this.options.enforceRestrictions && descriptor.type) {
          ...
          if (schema) {
            let type = schema.simpleTypes[descriptor.type.name]; // if type not available in global schema
      ...
      if (val != null && (typeof val !== 'object' || val instanceof Date)) {
        ...
        val = toXmlDateOrTime(descriptor, val);
        element.text(val); //add $attributes. Attribute can be an attribute defined in XSD or an xsi:type.
  ...
  getXsiType(descriptor, attrs) {
    ...
    if (attrs != null && typeof attrs === "object") {
      for (let p in attrs) {
        let child = attrs[p]; // if field is $xsiType add xsi:type attribute
        ...
        if (p === this.options.xsiTypeKey) {
          ...
          if (schema) {
            var xsiTypeInfo = schema.complexTypes[xsiType.name] || schema.simpleTypes[xsiType.name]; // The type might not be described
  ...
  _processText(top, val) {
    ...
    if (top.object === null) return; // Top object has no other elements or attributes
  ...
  xmlToJson(nsContext, xml, descriptor) {
    ...
    p.onopentag = function (node) {
      ...
      var obj = undefined;
      var elementAttributes = null; // Register namespaces 1st
      ...
      for (let a in attrs) {
        ...
        let attrValue = parseValue(attrs[a], attrDescriptor); // if element attribute is xsi:type add $xsiType field
      ...
      elementQName.nsURI = nsContext.getNamespaceURI(elementQName.prefix); // SOAP href (#id)
    ...
    p.write(xml).close(); // merge obj with href
    ...
    if (root.Envelope) {
      ...
      if (root.Envelope.Body !== undefined && root.Envelope.Body !== null) {
        if (body.Fault !== undefined && body.Fault !== null) {
          ...
          var errorMessage = getSoap11FaultErrorMessage(body.Fault); //check if fault is soap 1.2 fault
...
function getSoap11FaultErrorMessage(faultBody) {
  ...
  if (faultcode) {
    //soap 1.1 fault
    errorMessage = ' '; //All of the soap 1.1 fault elements should contain string value except detail element which may be a complex type or plain text (string)
...
function parseValue(text, descriptor) {
  ...
  if (jsType === Date) {
    var dateText = text; // Checks for xs:date with tz, drops the tz 
...
module.exports = XMLHandler; // Exported function for testing
