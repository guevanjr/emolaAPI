...
class Server extends Base {
  ...
  _process(input, req, callback) {
    ...
    try {
      if (binding.style === 'rpc') {
        ...
        var operations = binding.operations; //figure out the output name

        for (var name in operations) {
          var inputParts = operations[name].input.message.parts; //find the first part of the input message. There could be more than one parts in input message.
          ...
          if (firstInPart.element.$name === messageElemName) {
            ...
            if (operations[name].output != null) {
              var outPart = operations[name].output.message.parts; //there will be only one output part
  ...
  _executeMethod(options, req, callback) {
    ...
    try {
      ...
      debug('Server executeMethod: error: %s ', error.message); //fix - should create a fault and call sendError (..) so that this error is not lost and will be sent as Fault in soap envelope
      //to the client?
    ...
    function handleResult(error, result) {
      ...
      var message = envelope.body.toString({
        pretty: true
      });
      var xml = envelope.doc.end({
        pretty: true
      });
  ...
  _envelope(env, includeTimestamp) {
    ...
    var soapHeaderElement = env.header; //add soapHeaders to envelope. Header can be xml, or JSON object which may or may not be described in WSDL/XSD.
  ...
  _sendError(operation, error, callback, includeTimestamp) {
    ...
    debugDetail('Server sendError. operationDescriptor: %j ', operationDescriptor); //get envelope descriptor
    ...
    var nsContext = self.createNamespaceContext(soapNsPrefix, soapNsURI);
    var envelope = XMLHandler.createSOAPEnvelope(soapNsPrefix, soapNsURI); //find the envelope body descriptor

    var bodyDescriptor = faultEnvDescriptor.elements[1]; //there will be only one <Fault> element descriptor under <Body>

    var faultDescriptor = bodyDescriptor.elements[0];
    debugDetail('Server sendError. faultDescriptor: %j ', faultDescriptor);
    debug('Server sendError.  error.Fault: %j ', error.Fault); //serialize Fault object into XML as per faultDescriptor
